...
# Function to create cursor-specific launcher
create_cursor_launcher() {
    local launcher_dir="$HOME/.local/bin"
    local launcher_file="$launcher_dir/cursor-fcitx"
    
    print_status "Creating Cursor launcher with fcitx5 support..."
...
# Set fcitx5 environment variables for Cursor
...
# Launch Cursor with proper environment
exec /usr/bin/cursor "$@"
EOF
    ...
    print_success "Created Cursor launcher at $launcher_file"
    print_status "You can now use 'cursor-fcitx' command to launch Cursor with proper input method support"
}
...
# Function to configure fcitx5 for Brazilian Portuguese
configure_fcitx5_br() {
    local profile_file="$HOME/.config/fcitx5/profile"
    
    print_status "Configuring fcitx5 for Brazilian Portuguese..."
    ...
    # Create new profile with US International and Brazilian layouts
    cat > "$profile_file" << 'EOF'
[Groups/0]
# Group Name
Name=Default
...
EOF
    ...
    print_success "Configured fcitx5 with US International layout for Brazilian Portuguese support"
}
...
test_input_method() {
    ...
    print_status "2. Log out and log back in (or restart)"
    print_status "3. Launch Cursor with: cursor-fcitx"
    print_status "4. Try typing 'ção' - the 'ç' should work correctly"
    print_status "5. Try Right Alt + c to get 'ç' directly"
}

# Main function
main() {
    echo "===== Cursor Input Method Fix Script ====="
    echo ""
    ...
    print_status "Then use 'cursor-fcitx' command to launch Cursor with proper input support"
}
